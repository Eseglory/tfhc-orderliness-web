...
suite('Application HTTP integration (real PostgreSQL)', () => {
  ...
  beforeAll(async () => {
    ...
    const cycle = await app.get(AvailabilityService).openCurrentWeek();
    await db.weeklyAvailabilityCycle.update({ where: { id: cycle.id }, data: { state: 'OPEN', closesAt: new Date(Date.now() + 3600000) } });
    const passwordHash = await argon2.hash('E2ePassword!123');
    const a = await db.user.create({ data: { email: `admin-${run}@tfhc.org`, passwordHash, role: 'ADMIN', passwordAuthEnabled: true, emailVerifiedAt: new Date() } });
    ...
    const m = await db.user.create({ data: { email: `member-${run}@tfhc.org`, passwordHash, role: 'MEMBER', member: { create: { memberCode: `E2E-${run}`, firstName: 'Test', lastName: 'Member', phoneNumber: '08012345678' } } }, include: { member: true } });
    ...
    meeting = await db.meeting.create({ data: { title: 'E2E Meeting', categoryId: category.id, meetingDate: time(0), startTime: time(-5), expectedArrivalTime: time(0), attendanceOpenTime: time(-30), attendanceCloseTime: time(60), locationName: 'Test Venue', latitude: 6.6697906, longitude: 3.3581822, status: 'ACTIVE', qrSecret: 'test-only-secret' } });
  }, 30000);
  ...
  test('analytics aggregates completed services and enforces reporting access', async () => {
    ...
    const fixture = await db.meeting.create({data:{title:'Analytics fixture',categoryId:category.id,meetingDate:time(-90),startTime:time(-60),expectedArrivalTime:time(-70),attendanceOpenTime:time(-80),attendanceCloseTime:time(-10),locationName:'Church',latitude:6.6697906,longitude:3.3581822,status:'CLOSED'}});
  ...
  test('restricted event close-out excludes members outside its audience', async () => {
    const restricted = await db.meeting.create({data:{title:'Restricted close-out',categoryId:category.id,meetingDate:time(-60),startTime:time(-30),expectedArrivalTime:time(-45),attendanceOpenTime:time(-60),attendanceCloseTime:time(-1),locationName:'Church',latitude:6.6697906,longitude:3.3581822,status:'ACTIVE',visibility:'RESTRICTED',isCompulsory:true,audiences:{create:{memberId}}}});
  ...
  test('profile picture upload enforces content, size, ownership, persistence and removal', async () => {
    ...
    await request(app.getHttpServer()).post('/members/me/photo').set(auth(member)).attach('photo', Buffer.alloc(1 * 1024 * 1024 + 1), 'large.png').expect(413);
    ...
    const exactLimit = Buffer.concat([image, Buffer.alloc(1 * 1024 * 1024 - image.length)]);
  ...
  test('admin edits personal details and deactivates/reactivates without losing history', async () => {
    ...
    await request(app.getHttpServer()).put('/members/me/profile').set(auth(member)).send({ email: 'changed@tfhc.org' }).expect(400);
    const profile = await request(app.getHttpServer()).get('/members/me/profile').set(auth(member)).expect(200);
    expect(profile.body.birthday).toBe('03-14');
  });
  test('admin login and bad credentials', async () => {
    await request(app.getHttpServer()).post('/auth/login').send({ email: `admin-${run}@tfhc.org`, password: 'E2ePassword!123' }).expect(201);
    await request(app.getHttpServer()).post('/auth/login').send({ email: `admin-${run}@tfhc.org`, password: 'wrong' }).expect(401);
  });
  test('malformed login is a client error', async () => { await request(app.getHttpServer()).post('/auth/login').send({}).expect(400); });
  test('members must use Google login', async () => { await request(app.getHttpServer()).post('/auth/login').send({ email: `member-${run}@tfhc.org`, password: 'E2ePassword!123' }).expect(403); });
  ...
  test('location-only check-in rejects outside/inaccurate GPS and duplicate attendance', async () => {
    const body = { meetingId: meeting.id, latitude: 6.6697906, longitude: 3.3581822, gpsAccuracy: 5 };
  ...
  test('outside attendance window returns 400', async () => {
    ...
    await request(app.getHttpServer()).post('/attendance/check-in').set(auth(member)).send({meetingId:meeting.id,latitude:6.6697906,longitude:3.3581822}).expect(400);
  ...
  test('absence requests validate input, remain private and reject without excusing attendance', async () => {
    ...
    const event = await db.meeting.create({data:{title:'Absence rejection',categoryId:category.id,meetingDate:time(0),startTime:time(30),expectedArrivalTime:time(0),attendanceOpenTime:time(0),attendanceCloseTime:time(60),locationName:'Church',latitude:6.6697906,longitude:3.3581822}});
  ...
  test('excuse approval and duplicate attendance with null arrival', async () => {
    ...
    await request(app.getHttpServer()).post('/attendance/check-in').set(auth(member)).send({meetingId:meeting.id,latitude:6.6697906,longitude:3.3581822}).expect(409);
  });
  test('corrections reject malformed input and meetings outside the member audience', async () => {
    const endpoint = '/excuses/corrections';
    const payload = { meetingId: meeting.id, reason: 'Attendance needs review', requestedStatus: 'EARLY' };
    await request(app.getHttpServer()).post(endpoint).set(auth(member)).send({}).expect(400);
    await request(app.getHttpServer()).post(endpoint).set(auth(member)).send({ ...payload, reason: ' ' }).expect(400);
    await request(app.getHttpServer()).post(endpoint).set(auth(member)).send({ ...payload, requestedStatus: 'INVALID' }).expect(400);
    await request(app.getHttpServer()).post(endpoint).set(auth(admin)).send(payload).expect(403);
    const hidden = await db.meeting.create({ data: { title: 'Private correction fixture', categoryId: category.id, meetingDate: time(-10), startTime: time(-10), expectedArrivalTime: time(-15), attendanceOpenTime: time(-20), attendanceCloseTime: time(30), locationName: 'Church', latitude: 6.6697906, longitude: 3.3581822, visibility: 'RESTRICTED' } });
    await request(app.getHttpServer()).post(endpoint).set(auth(member)).send({ ...payload, meetingId: hidden.id }).expect(404);
    expect(await db.correctionRequest.count({ where: { meetingId: hidden.id } })).toBe(0);
  });
  test('correction approval recalculates points and writes audit', async () => {
    const before = (await request(app.getHttpServer()).get('/scoring/leaderboard?limit=1000').set(auth(member)).expect(200)).body.find((row:any) => row.memberId === memberId);
    ...
    const after = (await request(app.getHttpServer()).get('/scoring/leaderboard?limit=1000').set(auth(member)).expect(200)).body.find((row:any) => row.memberId === memberId);
  ...
  test('member creation, search and update', async () => {
    ...
    const created = await request(app.getHttpServer()).post('/members').set(auth(admin)).send({firstName:'Created',lastName:run,phoneNumber:'08012345678',email:`created-${run}@tfhc.org`,subTeamId:team.body.id}).expect(201);
  ...
  test.each([
    ...
    for (const [index, status] of statuses.entries()) {
      ...
      const fixture = await db.meeting.create({ data: { title: 'Streak fixture', categoryId: category.id, meetingDate: date, startTime: date, expectedArrivalTime: date, attendanceOpenTime: date, attendanceCloseTime: date, locationName: 'Test', latitude: 6.6697906, longitude: 3.3581822, status: 'CLOSED' } });
    ...
    // Direct fixture writes bypass the invalidation performed by attendance endpoints.
    app.get(CacheService).invalidateTags(['scoring', 'leaderboard']);
  ...
  test('only admins manage Google approval, with audit and duplicate protection', async () => {
    const created = await request(app.getHttpServer()).post('/members').set(auth(admin)).send({firstName:'Approved',lastName:'Member',phoneNumber:'08012345678',email:`approved-${run}@tfhc.org`}).expect(201);
    const id = created.body.id;
    expect(await db.approvedMember.count({where:{memberId:id,status:'ACTIVE'}})).toBe(1);
    await request(app.getHttpServer()).put(`/members/${id}/google-access`).set(auth(member)).send({email:`approved-${run}@tfhc.org`,status:'REVOKED'}).expect(403);
    await request(app.getHttpServer()).put(`/members/${id}/google-access`).set(auth(admin)).send({email:`approved-${run}@tfhc.org`,status:'REVOKED'}).expect(200);
    ...
    await request(app.getHttpServer()).post('/members').set(auth(admin)).send({firstName:'Duplicate',lastName:'Email',phoneNumber:'08012345678',email:`approved-${run}@tfhc.org`}).expect(409);
    await request(app.getHttpServer()).put(`/members/${memberId}/google-access`).set(auth(admin)).send({email:'other@tfhc.org',status:'ACTIVE'}).expect(400);
    const leader = await db.user.create({data:{email:`leader-${run}@tfhc.org`,passwordHash:'not-used',role:'LEADER'}});
    const leaderToken = app.get(AuthService).generateToken(leader.id,leader.email,leader.role);
    await request(app.getHttpServer()).put(`/members/${id}/google-access`).set(auth(leaderToken)).send({email:`approved-${run}@tfhc.org`,status:'ACTIVE'}).expect(403);
    await request(app.getHttpServer()).post('/members').set(auth(leaderToken)).send({firstName:'Leader',lastName:'Approval',phoneNumber:'08012345678',email:`leader-approved-${run}@tfhc.org`}).expect(403);
  });
  ...
  test('manual closure creates summary and prevents reopening', async () => {
    const created = await db.meeting.create({data:{title:'Manual close',categoryId:category.id,meetingDate:time(0),startTime:time(-5),expectedArrivalTime:time(0),attendanceOpenTime:time(-10),attendanceCloseTime:time(60),latitude:6.6697906,longitude:3.3581822,locationName:'Test Venue',status:'ACTIVE'}});
  ...
  test('simultaneous check-ins create one record and return conflicts, never 500', async () => {
    const target = await db.meeting.create({data:{title:'Concurrent check-in',categoryId:category.id,meetingDate:time(0),startTime:time(-5),expectedArrivalTime:time(0),attendanceOpenTime:time(-10),attendanceCloseTime:time(60),latitude:6.6697906,longitude:3.3581822,locationName:'Test Venue',status:'ACTIVE'}});
    const responses = await Promise.all(Array.from({length:12},()=>request(app.getHttpServer()).post('/attendance/check-in').set(auth(member)).send({meetingId:target.id,latitude:6.6697906,longitude:3.3581822,gpsAccuracy:5})));
  ...
  describe('Google identity verification (provider responses simulated)', () => {
    ...
    test('simultaneous first Google logins link one usable account', async () => {
      const email = `concurrent-google-${run}@tfhc.org`;
    ...
    test('newly provisioned member can sign in and admin revocation invalidates the session', async () => {
      const email = `onboard-${run}@tfhc.org`;
