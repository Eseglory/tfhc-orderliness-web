...
import { Award, Flame, Trophy, Medal } from 'lucide-react';
...
export default function LeaderboardPage() {
  ...
  useEffect(() => {
    async function loadLeaderboard() {
      try {
        const data = await fetchApi('/scoring/leaderboard');
        setLeaderboard(data);
      } catch (err) {
        console.error(err);
      } finally {
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6 space-y-6">
        <div className="text-center max-w-xl mx-auto">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-amber-500/10 text-amber-400 text-xs font-semibold mb-2 border border-amber-500/20">
            <Trophy className="w-4 h-4" /> Official Unit Participation Leaderboard
          </div>
          <h1 className="text-3xl font-extrabold text-white">Participation Leaderboard</h1>
          <p className="text-xs text-slate-400 mt-1">
            Composite Score = (Attendance Rate × 60%) + (Punctuality Rate × 40%)
          </p>
        </div>
                ...
                {leaderboard.length > 0 ? (
                  ...
                  <tr>
                    <td colSpan={8} className="px-6 py-12 text-center text-slate-500">
                      {loading ? 'Calculating unit performance ratings...' : 'No active members rated yet.'}
                    </td>
                  </tr>
                )}
              </tbody>
